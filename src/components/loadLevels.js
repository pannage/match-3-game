...
function getBoardDataOfLevel1() {
    return getNewBoarDataOfGame().map((row, rowId) => {
        return row.map((cell, cellId) => {
            ...
            // cell.isDesk = Math.random() * 100 > 50;
            // cell.isDesk = true;
...
function getBoardDataOfLevel4() {
    return getNewBoarDataOfGame().map((row, rowId) => {
        return row.map((cell, cellId) => {
            if (cellId === 0 || (cellId === 1 && rowId > 3) || (cellId === 6 && rowId > 3) || cellId === 7) {
                return {
                    url: 'url(../images/ground.png)',
                    type: 'ground',
                    toDelete: false,
...
function getBoardDataOfLevel5() {
    return getNewBoarDataOfGame().map((row, rowId) => {
        return row.map((cell, cellId) => {
            if (cellId === 0 || (cellId === 1 && rowId > 1) || (cellId === 2 && rowId > 3) || (cellId === 3 && rowId > 5)) {
                return {
                    url: 'url(../images/ground.png)',
                    type: 'ground',
                    toDelete: false,
...
function getBoardDataOfLevel7() {
    return getNewBoarDataOfGame().map((row, rowId) => {
        return row.map((cell, cellId) => {
            if ((rowId > 4 && (cellId === 2 || cellId === 5)) || (rowId > 3 && (cellId === 3 || cellId === 4)) || rowId === 7) {
                return {
                    url: 'url(../images/ground.png)',
                    type: 'ground',
                    toDelete: false,
